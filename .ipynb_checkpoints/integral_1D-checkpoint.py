...
def posterior_log_np(b, data):
    ...
    for step in data.keys():
        angs = np.array(data[step]['angs'])
        t_os = np.array(data[step]['days_start'])
        t_fs = np.array(data[step]['days_end'])
        rvar = np.array(data[step]['rvar'])
        stds = np.array(data[step]['stds'])
        N = len(angs)

        #Calculate some variables
        sred = (st**-2 + rvar)**-1
        
        vec = angs - (a/(2*np.pi*f*(t_fs-t_os)))*(np.cos(2*np.pi*f*t_os + ph) - np.cos(2*np.pi*f*t_fs + ph))
        x_avg = np.mean(vec/stds**2)
        xsq_avg = np.mean((vec/stds)**2)
...
if __name__ == '__main__':
    ...
    sl = []
    def posterior_quad(ph):
        """Just a function of the phase to be integrated over"""
        return np.exp(posterior_log([a, f, ph, st], dt, prior_lims)+40)
    #Integrate with scipy quad over ph.
    ans = quad(posterior_quad,0, 2*np.pi, epsrel = accuracy)
